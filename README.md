...
- **中断・再開**: 通常学習・条文連続復元・模試の入力を保持し、ホームの「続きから再開」から戻れます。同じタブでの再読み込みにも対応します。時間制限のあるモードは中断中も時間が進みます。通常学習は新しいセッションを始めると再開対象が置き換わります。
...
憲法だけを更新する場合は `npm run migrate -- --constitution-only` を使えます。既存bundleの政経・無機化学は保持したまま、憲法の原本を検証して再生成します。
